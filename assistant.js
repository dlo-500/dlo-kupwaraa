/*!
 * DLO Kupwara Assistant · NK.2.2 — dlokupwara.in
 *
 * WHAT IT DOES
 ...
 *  - LAW DESK: civil-procedure Q&A — CPC, plaint, written statement, caveat, ex parte, restoration,
 *    condonation of delay, limitation, appeals, execution, contempt, S.80 notice, consumer/MACT and more.
 *    Answers stream in like a chat model; typing shows topic suggestions.
 *
 * SETUP: none. Load it once, at the end of index.html:   <script src="assistant.js"></script>
 * It auto-detects the page's data and Supabase connection. (Optional overrides, BEFORE the script tag:
 *   window.DLO_ASSISTANT_CONFIG = { isStaff: () => true, logTable: "assistant_misses", pages: {...} };)
 *
...
(function () {
  ...
  // Page globals from index.html (classic-script consts/lets are visible by name; typeof-guarded so other pages/Node are fine)
  ...
  const FLAGLABEL = { exparte: "Ex-parte", missing: "Reply not filed, hearing within 7 days", pending: "Reply not filed", filed: "Reply filed", overdue: "Overdue (hearing date passed, still Active)", disposed: "Disposed", active: "Active" };
  ...
  const todayD = () => { const d = new Date(new Date().toLocaleString("en-US", { timeZone: "Asia/Kolkata" })); return new Date(d.getFullYear(), d.getMonth(), d.getDate()); };   // IST, like the portal
  ...
  function toRow(raw) {
    ...
    let nd = null;   // the portal's ALL_ROWS carries next hearing as a Date object
    ...
    const r = { title: g(K.title), number: g(K.number), subject: g(K.subject), dept: g(K.dept), court: g(K.court), last: g(K.last), nextRaw: nd ? "" : g(K.next), status: g(K.status), reply: g(K.reply), type: g(K.type), counsel: g(K.counsel) };
    const ex = g(K.exparte).toLowerCase();
    r.next = nd || parseDateStr(r.nextRaw);
    const st = r.status.toLowerCase();                     // same tests the portal uses
    r.disposed = /dispos|decided|dismiss|closed|withdrawn|struck/.test(st);
    r.active = st.indexOf("active") !== -1;
    const rp = r.reply.toLowerCase();
    r.replyState = rp.indexOf("not") !== -1 || /pend|await|\bnil\b/.test(rp) ? "pending" : /filed|submit|done|yes/.test(rp) ? "filed" : "";
  ...
  async function fetchRows() {
    const pr = pageRows(); if (pr) return { list: pr, source: "page" };
    const cl = getClient();
    if (cl) {
      try {
        let all = [];
    ...
    for (const n of ["allCases", "cases", "casesData", "DLO_CASES"]) if (Array.isArray(W[n]) && W[n].length && typeof W[n][0] === "object") return { list: W[n], source: "page" };
    if (HAS_DOM) { const t = scrapeTables(); if (t.length) return { list: t, source: "page-table" }; }
    return { list: [], source: "none" };
  }
  function syncFromPage() {   // the portal refreshes ALL_ROWS itself; follow it so the bot never disagrees with the screen
    const pr = pageRows();
    if (pr && (pr !== DATA.pageRef || pr.length !== DATA.pageLen)) { setRows(pr); DATA.pageRef = pr; DATA.pageLen = pr.length; DATA.source = "page"; DATA.at = Date.now(); }
  }
  async function fetchUpdates() {   // notice board (announcements table) — same public read the portal uses
    const cl = getClient(), c = creds();
    try {
      if (cl) { const r = await cl.from("announcements").select("*").order("created_at", { ascending: false }).limit(10); if (!r.error && r.data) return r.data; }
      else if (c.url && c.key && typeof fetch === "function") {
        const res = await fetch(c.url + "/rest/v1/announcements?select=*&order=created_at.desc&limit=10", { headers: { apikey: c.key, Authorization: "Bearer " + c.key } });
  ...
  function loadExtras() {
    ...
    fetchUpdates().then((rows) => {
      DATA.updates = (rows || []).map((x) => ({ date: x.date_label || (x.created_at ? new Date(x.created_at).toLocaleDateString("en-IN") : ""), category: x.category || "", title: x.title || "", desc: x.description || "" })).filter((u) => u.title.trim());
    }).catch(() => {});
  }
  function ensureData(force) {
    syncFromPage(); loadExtras();
    ...
    DATA.loading = Promise.race([fetchRows(), timeout])
      .then((res) => { if (res.list.length) { setRows(res.list); DATA.source = res.source; if (res.source === "page") { DATA.pageRef = pageRows(); DATA.pageLen = DATA.pageRef ? DATA.pageRef.length : 0; } } })
      .catch(() => {})
  ...
  function rowPass(r, f) {
    ...
    if (f.flag) {
      ...
      switch (f.flag) {   // same definitions as the portal's filters, overdue and missing-reply lists
  ...
  function computeStats(rows) {
    ...
    rows.forEach((r) => {
      if (r.disposed) s.disposed++; else if (r.active) s.active++; else if (/pending/i.test(r.status)) s.pendingStatus++;
  ...
  function filterReply(f, wantList) {
    ...
    let head = "**" + title + "**\nTotal " + st.total + " · Active " + st.active + " · Disposed " + st.disposed + (st.pendingStatus ? " · Pending " + st.pendingStatus : "") + (st.total ? " (" + st.rate + " disposed)" : "") + "\nReply not filed " + st.replyPending + " · Filed " + st.replyFiled + " · Ex-parte " + st.exparte;
  ...
  function generalData(q) {
    ...
    if (rank && /(dept|department|repl|pend)/.test(q)) {
      const g = group(rows.filter((r) => r.replyState === "pending"), deptLabel).slice(0, 7);
      return R("**Most replies not filed**\n" + g.map((x, i) => (i + 1) + ". " + x.k + " — " + x.n).join("\n"), { link: PAGES.performance, chips: [C("Department overview", "all departments"), MENU] });
    }
  ...
  /* ── Retrieval engine ── */
  "process procedure procedural steps stages limitation grounds provision law legal cpc apply explain explanation meaning define definition section sec rule difference differences differ allowed power jurisdiction punishment penalty defence defense remedy remedies hota hoti hote karte karta karna karein kare kaise kese kaisay under work works simple words bare sir file lodge served serve received receive receiving history proceeding proceedings previous past timeline happened".split(" ").forEach((w) => { NOISE[w] = 1; });
  ...
  function histReply(rem) {   // proceedings history from the portal's own HISTORY_ROWS
  ...
  function answer(raw) {
    ...
    if (rem.length && /\b(history|proceedings?|previous hearings?|past hearings?|what happened|timeline)\b/.test(qFull)) { const hh = histReply(rem); if (hh) return hh; }
  ...
  let closeHook = null;
  const pageRowFor = (r) => { const pr = pageRows(); return pr ? (pr.filter((x) => x.title === r.title && (x.caseNo || "") === r.number)[0] || pr.filter((x) => x.title === r.title)[0] || null) : null; };
  function buildCard(r, hl) {
    ...
    if (typeof openCasePopup === "function" && pageRowFor(r)) {   // the portal's own case-detail popup
      const fdb = h("button", "dlo-mini", "Full details"); fdb.type = "button";
      fdb.addEventListener("click", () => { const pr = pageRowFor(r); if (pr) { if (closeHook) closeHook(); openCasePopup(pr); } });
      act.appendChild(fdb);
    }
  ...
  function mount() {
    ...
    function addBot(rep) {
      ...
      const finish = () => {
        ...
        if (rep.link && rep.link.url) { b.appendChild(document.createElement("br")); const a = h("a", "dlo-bubble-action", (rep.link.text || "Open") + " →"); a.href = rep.link.url; if (String(rep.link.url).charAt(0) === "#") a.addEventListener("click", () => { close(); }); b.appendChild(a); }
    ...
    function placeUI() {   // sit above the portal's mobile bottom tab bar when it is showing
      let off = 0;
    ...
    placeUI(); ensureData();   // warm the data in the background
  }
