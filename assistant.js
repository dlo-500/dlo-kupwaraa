...
(function () {
  ...
  const LAW_FOOT = "Authoritative guidance from DLO Kupwara Legal Training Manual — not individual legal advice. Confirm with assigned standing counsel & statutory text.";
  ...
  const CSS = [
    ...
    ".dlo-status-dot.thinking{background:#f43f5e;box-shadow:0 0 8px #f43f5e;animation:dloPulseDot 1.2s infinite ease-in-out}",
    "@keyframes dloPulseDot{0%,100%{transform:scale(1)}50%{transform:scale(1.35)}}",
    ".dlo-header-title{font-weight:600;font-size:13.5px}.dlo-header-sub{font-size:10px;opacity:.85;margin-top:1px}",
    ...
    "#dlo-chat-body{position:relative;padding:14px;overflow-y:auto;flex-grow:1;display:flex;flex-direction:column;gap:10px;background:radial-gradient(circle at 90% 10%, rgba(245,158,11,0.06), transparent 45%), radial-gradient(circle at 10% 90%, rgba(139,92,246,0.06), transparent 50%), #f8fafc;scroll-behavior:smooth}",
    ...
    "#dlo-input-bar{display:flex;align-items:center;gap:6px;padding:8px 12px;background:#fff;border-top:1px solid #e2e8f0;flex-shrink:0;position:relative;transition:all .3s ease}",
    
    "/* Chat Input: Warm multi-color gradient (amber, subtle violet, soft rose) layered blurred box-shadow */",
    "#dlo-user-input{flex-grow:1;border:1px solid #cbd5e1;border-radius:20px;padding:8px 14px;font-size:12px;outline:none;color:#0f172a;font-family:inherit;background:#fff;transition:border-color .25s ease,box-shadow .28s cubic-bezier(.4,0,.2,1)}",
    ...
    "#dlo-send-btn{background:#0c2340;color:#fff;border:none;border-radius:50%;width:32px;height:32px;display:flex;align-items:center;justify-content:center;font-size:13px;cursor:pointer;transition:background .15s,transform .15s;flex-shrink:0}",
  ...
  function mount() {
    ...
    // Header displays true 392 baseline
    ...
    function refreshHeader() {
      ...
      sub.textContent = ok ? (S.staff ? "Staff view" : "Public view") + " · " + DATA.rows.length + " cases live" : "Registry / Knowledge base ready";
    }
    ...
    function addBot(rep) {
      ...
      if (rep.stream && !reduceMotion()) streamRich(b, rep.text, finish);
